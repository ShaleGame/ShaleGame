...
public class CharacterInventoryIntegrationTest : IDisposable
{
    ...
    [Fact]
    public void GivenCharacterWithClone_WhenCloneReady_DoesNotChangeOriginalEquippedWeapon()
    {
        var inventory = _character.Inventory;

        // Equip the second launcher on the original character
        inventory.EquipWeapon(_rocketLauncher2);

        // Split the character to create a clone and run the clone's _Ready
        var clone = _character.Cloneable.Split();
        clone.Inventory._Ready();

        // Original should still have the second launcher equipped
        inventory.IsWeaponEquipped(_rocketLauncher2).ShouldBeTrue();

        // Clone should equip a weapon of the same kind, but not the same instance
